...
function checkCookie (client) {
    return client.jsessionid && client.rno ? Promise.resolve() : Promise.reject(Error.needCookie);
}

JWXT.prototype.getCookie = function() {
...
JWXT.prototype.getCheckCode = function() {

    var client = this;

    function handleError (err) {
        if (Error.needCookie.code == err.code) {
            return JWXT.prototype.getCookie.apply(client);
        } else {
            throw err;
        }
    }
...
JWXT.prototype.login = function(username, password, checkcode) {
...
JWXT.prototype.logout = function () {
...
JWXT.prototype.isLogin = function() {
...
JWXT.prototype.getTimeTable = function(year, term) {
...
function solveStrangeJSON(json) {
    ...
    for (var courseMatch = coursesRegex.exec(primary); courseMatch; courseMatch = coursesRegex.exec(primary)) {
        ...
        for (var keyValueMatch = keyValueRegex.exec(courseStr); keyValueMatch; keyValueMatch = keyValueRegex.exec(courseStr)) {
            var remove = /,|\\/;
            var key = keyValueMatch[1].replace(remove, ''), value = keyValueMatch[2].replace(remove, '');
            course[key] = value;
        }
        courses.push(course);
    }
...
JWXT.prototype.getScore = function(year, term) {
...
JWXT.prototype.getElectResult = function(year, term) {
